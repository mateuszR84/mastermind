...
The contact form submits to Formspree (`action="https://formspree.io/f/xyegaqlo"`, plain `POST`, no JS) — the account and endpoint belong to the client. Field names (`imie`, `telefon`, `email`, `przedmiot`, `wiadomosc`) surface as-is in Formspree's notification email; a hidden `_subject` field sets the email subject line. A hidden `_next` field keeps the user on-site after submit instead of redirecting to Formspree's hosted confirmation page — it must be an **absolute URL** (`https://szkola-mastermind.pl/#kontakt-dziekujemy`), not a relative path: the redirect is an HTTP `Location` header from Formspree's own response, which browsers resolve against `formspree.io`, not the referring page, so a root-relative value silently sends the user to Formspree's domain instead. The domain must also be added under Allowed Domains in the Formspree dashboard. The `#kontakt-dziekujemy` target is a `.form-success` banner inside the form, shown via CSS `:target` (no JS) when the browser lands back on that fragment. Formspree still handles error display on its own hosted page, since there's no JS to intercept failed submissions client-side.
