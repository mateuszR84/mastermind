...
- **Design tokens live in `:root`.** Never hardcode a color that already has a variable. `--mint` (#7CE8B4) is the single accent — CTA buttons (`.btn-primary`/`.btn-outline`), nav links, the footer social button, and icon colors all use it; darker mint shades are derived via `color-mix`. `--placeholder` (amber, #E8B45E) is reserved for two things: (1) unfinished bracketed content via `.placeholder-tag`, and (2) plain inline text links within body copy — currently just `.fb-link` in `#kontakt` — per client preference. It is deliberately **not** used for nav links, buttons, or the footer; those stay mint even though they're also `<a>` tags. Don't reintroduce amber there without the client asking again — this has been tried and reverted once already.
...
- **Two icon systems coexist.** Most icons are hand-written inline SVGs stroked with `stroke="var(--mint)"`. A **Font Awesome 6.7 kit** (free tier, CSS/webfont method) is loaded in `<head>`, so `<i class="fa-brands fa-facebook">` also works — used by the footer social icon and the four `#kontakt` info-row icons (`fa-location-dot`, `fa-phone`, `fa-envelope`, `fa-facebook`), all mint. Font Awesome icons take color from CSS `color`, not `fill`, so they need a rule like `.social-btn i { color: var(--mint) }` rather than the `fill` attribute the inline SVGs use. Only free icons (`fa-solid`, `fa-regular`, `fa-brands`) are available.
...
The contact form has **no `action` or backend** — it does not send anything. A form service (Formspree/EmailJS/custom endpoint) must be wired up before launch; see the comment above the `<form>`.
